...
만약 여기에서 $x_{t+1}$에 대해 곧바로 새로운 중요도 샘플링을 수행한다면 다음과 같이 된다.
...
이로부터 포인트 $\{x_t^{(i)}, w_t^{(i)}\}$는 $p(x_t | z_t, z_{t-1}, \cdots, z_0)$의 근사로 볼 수 있다.
