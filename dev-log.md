...
블로그 SEO가 다 깨진 걸 발견했다. URL을 날짜를 포함하도록 바꾼 이후다. 속상하다. 근데 기왕 이렇게 된 거, 지금의 Serverless 구조에서 Serverful(?) 구조로 업데이트하기로 한다.
기존의 구조는 유지보수가 너무 어렵다.

새로운 구조에서 포스트에는 어떻게 접근하는 것이 좋겠는가?

- 데이터베이스?
- 아니면 지금처럼 파일시스템 기반으로?
- 파일시스템 기반으로 Git에 트랙하는 것은 좋다.
- 그런데 Kubernetes 등에 배포할 것 같은데 그렇다면 hostPath를 사용하는 것이 좋을까?

## 2023 / 09 / 02

Koa / React / TS / Vite 기반으로 블로그를 리팩토링했다. Kubernetes 배포했고, LocalPV를 사용했다.

## 2024 / 02 / 15

블로그 디자인을 완전 단순하게 개선했다.

## 2024 / 02 / 16

궁극의 디자인이란 어떤 것인가? 이미지, 영상을 비롯해서 이것저것 많은 내용이 포함된 컨텐츠를 파일 기반으로 쉽게 관리할 수 있는 방법은 무엇인가? 이것은 대단히 어려운 질문이라 할 수 있다.

## 2024 / 03 / 14

블로그에서 백엔드를 완전히 제거하고 템플릿 엔진 기반으로 개선했다. 많은 것이 편해진다.

- NGINX를 제외한 별도의 런타임이 필요없으므로 도커를 관리할 필요가 없다.
  - harbor, sealed secret, docker config json, image pull secret...etc.
- 타입스크립트 빌드를 할 필요가 없으므로 빌드 시간이 단축된다. (1.5초 내외)
- 일부 라이브러리를 제외하면 JS가 전혀 없으므로 로딩 속도가 빨라진다.
- SEO에 대단히 유리하다.
